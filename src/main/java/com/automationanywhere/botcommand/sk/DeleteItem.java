...
@BotCommand
@CommandPkg(label = "Delete Item", name = "DeleteItem", description = "Delete Item", 
icon = "pkg.svg", node_label = "Delete Item {{sessionName}}", comment = true ,  text_color = "#FF9900" , background_color =  "#FF9900" ,
...
public class DeleteItem{
    ...
    public StringValue action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session Name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName,
    	...
    	StringValue returnValue = new StringValue();
    	AWSDynamoDBConnection connection  = (AWSDynamoDBConnection) this.sessions.get(sessionName);  
    	
    	String result = connection.deleteItemDB(table, partitionkey,partitionvalue,sortkey,sortvalue);
